...
# Mock function for get_service_request_status
def get_service_request_status(request_id):
    """Mock service request status API that returns randomized status for a service request ID."""
    # Define possible service request statuses and service types
    statuses = ["pending", "in_progress", "completed", "cancelled"]
    service_types = ["account_opening", "loan_application", "credit_card", "complaint", "transaction_dispute"]

    # Generate random data based on the request ID to ensure consistency for the same ID
    # Using the sum of ASCII values of the request ID as a seed
    ...
    print(f"Service request status for {request_id}: {status}")
...
# Define a function tool for get_service_request_status
def service_request_status_tool(request_id: str):
    """Get the current status and details of a service request.

    Args:
        request_id: The service request ID to look up.

    Returns:
        Dictionary containing service request status details
    """
    return get_service_request_status(request_id)
...
async def audio_loop():
    ...
    agent = Agent(
        ...
        model=MODEL,
        instruction="You are a helpful customer service assistant for Cymbal Bank. You can help customers check the status of their service requests. When asked about a service request, you should ask for the request ID and then use the service_request_status_tool to retrieve the information. Be courteous, professional, and provide all relevant details about service types, completion dates, and current status.",
        tools=[service_request_status_tool],
    )
